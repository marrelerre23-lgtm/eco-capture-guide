-- Security Fix: Add explicit user_id validation to RPC functions
-- This prevents users from checking/incrementing other users' limits

-- Drop and recreate check_user_limits with auth validation
...
-- Drop and recreate increment_usage_counter with auth validation
DROP FUNCTION IF EXISTS public.increment_usage_counter(uuid, text);
